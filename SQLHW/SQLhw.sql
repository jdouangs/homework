...
CREATE VIEW total_staff AS
SELECT s.first_name, s.last_name, SUM(amount) as GROSS
FROM staff s
JOIN payment p
...
-- DROP VIEW total_staff;

SELECT * FROM total_staff;
-- 6c. List each film and the number of actors who are listed for that film. Use tables `film_actor` and `film`. Use inner join.
CREATE VIEW film_number AS
...
SELECT * FROM film_number;

-- 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system?
SELECT COUNT(film_id) AS 'Count of Copies of Hunchback Impossible'
FROM inventory i
WHERE film_id IN
	(
    SELECT film_id
